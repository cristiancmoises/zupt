...
The `v5.2.2`, `v5.2.3`, and `v5.2.4` tags are immutable non-promoted candidates.
...
packages and release assets must use `v5.2.5`; never move or overwrite an
earlier tag or checksum, and never transfer prior evidence automatically.
...
SOURCE_DATE_EPOCH="$(git show -s --format=%ct HEAD)" \
  make DIST_TARBALL=/tmp/zupt-5.2.5.tar.gz dist
...
| GUI DEB | `packaging/build-gui-deb.sh` | `zupt-gui_5.2.5_all.deb` after payload/dependency and installed integration gates |
| GUI RPM | `packaging/build-gui-rpm.sh` | `zupt-gui-5.2.5-1.noarch.rpm` and matching `.src.rpm` after package and installed integration gates |
| Linux CLI archive | `.github/workflows/ci.yml` | `zupt-5.2.5-linux-x86_64.tar.xz` with notices after dependency, member, and extracted functional gates |
| Portable GUI source | `packaging/portable/`, `.github/workflows/ci.yml` | `zupt-gui-5.2.5-portable.zip` after source scan, member allowlist, and extracted off-screen integration gate |
| Fedora / RPM-based systems | `packaging/rpm/zupt.spec` | downstream RPM starting point |
| AppImage helper | `packaging/build-appimage.sh` | downstream-only helper; no 5.2.5 AppImage is promoted |
...
`zupt-5.2.5-linux-x86_64.tar.xz` beside README, changelog, security guidance,
...
The `zupt-gui-5.2.5-portable.zip` artifact is source-only: it contains the GUI
...
AppImage creation is deliberately offline and is not a 5.2.5 release gate.
...
upstream 5.2.5 workflow. AppDir and Flatpak bundles and GUI platform installers
...
5.2.5 release evidence. Extended-length/device namespace paths, raw UNC output
roots, and mapped/network-drive output are not supported in 5.2.5. Publish the
...
publishing the immutable tag, update each recipe to version 5.2.5 and to the
...
1. start from the immutable `v5.2.5` tag;
...
- [ ] The source URL resolves to the immutable `v5.2.5` tag.
