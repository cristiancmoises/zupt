# ZUPT 5.2.6 for openSUSE Build Service
...
attribute upstream code or the 5.2.2/5.2.3/5.2.4/5.2.5/5.2.6 upstream changes to
Cabelo.
...
| File | Purpose |
|---|---|
| `_service` | Fetch the immutable `v5.2.6` tag and create `Source0` at build time. |
...
services reconstruct `zupt-5.2.6.tar.gz` inside the build environment, which
matches `Source0` in the spec.

This source policy does not prohibit separately built release-page packages.
The upstream 5.2.6 gates may publish the CLI source tarball, DEB, binary RPM,
...
`zupt-5.2.6.tar.gz` next to the spec and use a disposable RPM build tree:
...
local results do not establish 5.2.6, native Windows or macOS success, hosted
...
automatically to v5.2.6. The exact v5.2.6 candidate must repeat every applicable
...
1. Upstream completes every applicable pre-tag source and local audit gate,
   then creates and verifies the annotated `v5.2.6` tag. Exact-tag hosted,
...
2. With Git, `file`, bsdtar, tar, zip, unzip and SHA-256 tools installed, run
   `scripts/export-opensuse-package.sh v5.2.6`. Verify the reported ZIP and
...
4. From the extracted handoff root, run
   `packaging/opensuse/source-audit.sh --archive /path/to/zupt-5.2.6.tar.gz`.
